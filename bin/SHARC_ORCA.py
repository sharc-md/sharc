...
version='2.0'
versiondate=datetime.date(2018,4,1)
...
def execute_tinker(QMin,ff_file_path):
    ...
    * point charges
    ...
    while True:
        ...
        q=float(s[-1])
        print '  line %i' % iline,datetime.datetime.now()
        # find right atom
...
# --------------------------------------------- ORCA.resources ----------------------------------
    ...
    if 'theodore' in QMin:
        ...
        os.environ['THEODIR']=QMin['theodir']
        if 'PYTHONPATH' in os.environ:
          os.environ['PYTHONPATH']=os.path.join(QMin['theodir'],'lib')+os.pathsep + os.pathsep + os.environ['PYTHONPATH']
...
def writeORCAinput(QMin):
    ...
    # cpu cores
    if QMin['ncpu']>1 and not 'AOoverlap' in QMin:
      string+='%%pal\n  nprocs %i\nend\n\n' % (QMin['ncpu'])
    string+='%%maxcore %i\n\n' % (QMin['memory'])
