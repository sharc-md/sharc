...
as a list containing all initial condition objects."""
    ...
    if vibstate is not None:
        nvib = len(vibstate)
        if nvib < len(modes):
            print("ERROR: not enough quantum numbers given to -v! There should be at least %i quantum numbers." % (len(modes)))
            sys.exit(1)
        elif nvib > len(modes):
...
'''
    ...
    parser.add_option('-v', dest='v', type=str, nargs=1, default=None, help="Give an explicit list of vibrational states for all modes (e.g., '0 0 1 0 0 ...'). The string should contain one number for each considered mode after removing low-frequency/zero-norm modes. If used, temperature is ignored. ")
